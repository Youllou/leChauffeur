    ...
    async def ajoute_roleReaction(self, ctx, emoji, role):
        ...
        if ctx.author.guild_permissions.administrator:
            try:
                ...
                if ctx.message.reference is None:
                ...
                elif type(emoji) == discord.Emoji or emoji_pattern.search(emoji) is not None:
                    ...
                    newDat = [ctx.message.reference.id,emoji,int(role[3:-1])]
                    get_info.write(f"./assets/{str(ctx.guild.id)}/role_react.csv",'\a',old+newDat)