...
class help(commands.Cog):
    ...
    async def help(self,ctx):
        ...
        if (not ctx.author.bot) and str(ctx.channel.id) in command_chan:
            ...
            reactions = """stp ajoute_reaction [message/expression auquel réagir] [réactions+] (si un expression contient des espaces il faut la mettre entre guillemets, vous pouvez mettre plusieurs reactions en le séparant par des espaces)

stp retire_reaction [message/expression à supprimer]
...
stp shifu_class"""
            dancing = """stp join (tu dois etre dans un channel vocal)

stp play [url youtube / nom de la musique]
...
stp send [message] (pas besoin de guillemets)"""
            ...
            sentence.add_field(name="Dancing time",value=dancing)
